...
def compute_map(labels_and_predictions,
  ...
  if nms_on_tpu:
    def _not_padded(example):
      return ssd_constants.IS_PADDED not in example or not example[
          ssd_constants.IS_PADDED]

    detections = [
        example['detections']
        for example in labels_and_predictions
        if _not_padded(example)
    ]
    predictions = np.array(detections, dtype=np.float32).reshape((-1, 7))
